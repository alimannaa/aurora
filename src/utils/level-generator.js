...
export default class Level{
    ...
    placeRooms(rooms){
        for(let i = 0; i < rooms.length; i++){
            ...
            room.bottom = room.startCenter.y + Math.floor(height/2);
            
            // checking if the terrain for room is clear
            let sum = 0;
            if(room.top > 0 && room.left > 0 && room.bottom < this.height && room.right < this.width)
    ...
    quadTreeDropPoints(points, number, left, right, top, bottom){
        if(number === 0) return;
        if (number === 1){
            let x = 0, y = 0;
            let notAdded = true;
            while(notAdded) {
                ...
                y = Phaser.Math.RND.integerInRange(top, bottom);
            ...
            const yCenter = Math.floor((bottom + top) / 2);
            const quadrants = this.getQuadrantsNums(number, left, right, top, bottom, xCenter, yCenter);
            ...
            this.quadTreeDropPoints(points, quadrants[2], left, xCenter, yCenter+1, bottom);
            this.quadTreeDropPoints(points, quadrants[3], xCenter+1, right, yCenter+1, bottom);
        }
    }
