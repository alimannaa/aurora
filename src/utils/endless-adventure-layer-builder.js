...
export default function buildLevel(width, height, maxRooms, scene){
    ...
    for(let y = 0; y < height; y++)
        for(let x = 0; x < width; x++)
            ...
            else if(levelMatrix[y][x] === -1) groundLayer.putTileAt(TILE_MAPPING.ISLAND, x, y)
    ...
    rooms.forEach(room => {
        ...
        groundLayer.putTileAt(TILE_MAPPING.DUST.BOTTOM_RIGHT, right, bottom);
        groundLayer.putTileAt(TILE_MAPPING.DUST.BOTTOM_LEFT, left, bottom);
        for(let v = top + 1; v < bottom; v++){
            groundLayer.putTileAt(TILE_MAPPING.DUST.LEFT, left, v);
            groundLayer.putTileAt(TILE_MAPPING.DUST.RIGHT, right, v);
        }
        for(let h = left + 1; h < right; h++){
            groundLayer.putTileAt(TILE_MAPPING.DUST.TOP, h, top);
            groundLayer.putTileAt(TILE_MAPPING.DUST.BOTTOM, h, bottom);
        }

        // placing npc-s
        addNPCtoRoom(room, scene, groundLayer, stuffLayer, tilesize);
    });
        ...
        groundLayer.setCollision(TILE_MAPPING.WATER);
...
function addNPCtoRoom(room, scene, groundLayer, stuffLayer, tileSize)
{
    const x = Phaser.Math.Between(room.left, room.right) * tileSize;
    const y = Phaser.Math.Between(room.top, room.bottom) * tileSize;
    let wanderer = scene.characterFactory.buildCharacter( Phaser.Math.RND.integer() % 2 === 0 ? "punk" : "blue",
        x, y, {player: false});
    wanderer.addBehaviour(new SteeringDriven([ new Wander(wanderer) ])) ;
    scene.gameObjects.push(wanderer);
    scene.physics.add.collider(wanderer, groundLayer);
    scene.physics.add.collider(wanderer, stuffLayer);
};